...
public class JsonVectorStore(string basePath) : IVectorStore
{
        ...
        where TKey : notnull
    {
        ...
        {
            var propertyInfo = typeof(TRecord)
                .GetProperties()
                .Single(p => p.GetCustomAttribute<VectorStoreRecordKeyAttribute>() is not null
        ...
        private static Func<TRecord, ReadOnlyMemory<float>> CreateVectorReader()
        {
            var propertyInfo = typeof(TRecord)
                .GetProperties()
                .Single(p => p.GetCustomAttribute<VectorStoreRecordVectorAttribute>() is not null
